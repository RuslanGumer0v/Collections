...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            listAll.add(in.nextInt());
        }
        for (int i = 0; i < listAll.size(); i++) {
            if (listAll.get(i) % 3 == 0) listDivisibleByThree.add(listAll.get(i));
            if (listAll.get(i) % 2 == 0) listDivisibleByTwo.add(listAll.get(i));
            if (listAll.get(i) % 3 != 0 && listAll.get(i) % 2 != 0) listRemainder.add(listAll.get(i));
        }
        printList(listDivisibleByThree);
        printList(listDivisibleByTwo);
        printList(listRemainder);
    }
